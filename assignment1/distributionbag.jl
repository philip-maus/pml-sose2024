...
struct DistributionBag{T} <: AbstractArray{T, 1}
    ...
    DistributionBag{T}(uniform::T) where {T} = new(uniform, [])
end
...
```
"""
Base.firstindex(db::DistributionBag{T}) where {T} = Base.firstindex(db.bag)
...
```
"""
Base.lastindex(db::DistributionBag{T}) where {T} = Base.lastindex(db.bag)
...
julia> for d in db
 ...
 ```
"""
Base.iterate(db::DistributionBag{T}, i=1) where {T} = Base.iterate(db.bag, i)
